...
class WeightedL1Loss(nn.Module):
    ...
    def weighted_l1loss_channels(self, pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        result = torch.sum(weight * torch.abs(pred - target), dim=(-1, -2))
        result /= torch.sum(mask, dim=(-1, -2))
        return result

    def weighted_l1loss(self, pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        result = self.weighted_l1loss_channels(pred, target, mask)
        return torch.mean(result, dim=0)

    def forward(self, pred, target, mask):
        w_l1loss = self.weighted_l1loss(pred, target, mask)
        if self.sum:
