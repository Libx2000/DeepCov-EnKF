...
class OceanForecast(nn.Module):
    ...
    def __init__(
        ...
        norm_layer = partial(nn.LayerNorm, eps=1e-6)
        self.c = len(self.ocean_vars)
        self.h = self.img_size[0] // patch_size
        self.w = self.img_size[1] // patch_size
        self.feat_size = [sz // patch_size for sz in img_size]
        self.embed_dim = embed_dim_atmos + embed_dim_ocean
        self.num_layers = len(depths)
        self.constant = torch.from_numpy(load_constant(const_dir))

        # variable tokenization: separate embedding layer for each input variable
        self.var_map = self.create_var_map()
        self.patch_embed_atmos = nn.Conv2d(in_channels=len(self.atmos_vars),
                                     out_channels=embed_dim_atmos,
                                     kernel_size=patch_size,
                                     stride=patch_size)
        self.patch_embed_ocean = nn.Conv2d(in_channels=2*len(self.ocean_vars),
                                     out_channels=embed_dim_ocean,
                                     kernel_size=patch_size,
                                     stride=patch_size)
        self.num_patches = self.h * self.w
        self.in_norm = norm_layer(self.embed_dim, eps=1e-6)

        # positional embedding and lead time embedding
        self.lead_time_embed = nn.Sequential(
            nn.Linear(1, self.embed_dim, bias=True),
            nn.SiLU(),
        ...
        for i in range(self.num_layers):
            ...
            layers.append(layer)
            self.add_module(f"norm{i}", norm_layer(self.embed_dim, eps=1e-6))

        self.layers = nn.ModuleList(layers)

        self.fpn = nn.Sequential(
            nn.Linear(self.embed_dim * self.num_layers, self.embed_dim),
            nn.GELU(),
        )
        ...
        # prediction head
        self.head = nn.Linear(self.embed_dim, len(self.ocean_vars) * patch_size ** 2)
    ...
    def initialize_weights(self):
        ...
        wa = self.patch_embed_atmos.weight.data
        trunc_normal_(wa.view([wa.shape[0], -1]), std=0.02)
        wo = self.patch_embed_ocean.weight.data
        trunc_normal_(wo.view([wo.shape[0], -1]), std=0.02)
    ...
    def forward_encoder(self, x: torch.Tensor, lead_times, variables, noise=None):
        ...
        # (B, 8, H, W)
        atmos = x[:, :len(self.atmos_vars)]
        # (B, 13, H, W)
        ocean = x[:, len(self.atmos_vars):]

        # (B, C, H/p, W/p)
        in_atmos = self.patch_embed_atmos(atmos)
        in_ocean = self.patch_embed_ocean(ocean)
        # (B, 6C, H/p. W/p)
        h = torch.concat([in_atmos, in_ocean], dim=1)
        # (B, HW/p**2, 6C)
    ...
    def forward(self, x, mask, lead_times, variables, out_variables, noise=None):
        ...
        out_transformers = self.forward_encoder(torch.concat([constant, x, mask], dim=1), lead_times, variables, noise)  # B, L, D

        preds = self.head(out_transformers)  # B, L, V*p*p

        preds = self.unpatchify(preds)
        out_var_ids = self.get_var_ids(tuple(out_variables), preds.device)
        preds = preds[:, out_var_ids] + x[:, len(self.atmos_vars)-constant.shape[1]:]
